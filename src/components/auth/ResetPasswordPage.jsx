...
export default function ResetPasswordPage() {
    ...
    async function checkRecoverySession() {
      ...
      try {
        ...
        if (session?.user || recoveryType === 'recovery') {
          ...
          setError('重置链接无效、已过期，或当前未进入密码重置流程。');
        }
      } catch (sessionError) {
        if (!active) {
          return;
        ...
        setRecoveryReady(false);
        setError(normalizeFriendlyError(sessionError, '无法验证重置链接，请重新申请密码重置邮件。'));
      } finally {
        if (active) {
          setCheckingSession(false);
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccess('密码已更新。3 秒后将返回首页，您也可以立即手动跳转。');
      window.setTimeout(() => {
        navigate('/', { replace: true });
      }, 3000);
    } catch (updateError) {
      setError(normalizeFriendlyError(updateError, '密码更新失败，请重新打开重置邮件中的链接。'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="mx-auto max-w-md overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-2xl backdrop-blur">
        <div className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-950 px-6 py-5 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-wide">重置密码</h1>
              <p className="text-xs uppercase tracking-[0.3em] text-zinc-400">Password Recovery</p>
            </div>
          </div>
        </div>
