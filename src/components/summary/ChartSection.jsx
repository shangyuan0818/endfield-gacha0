 ...
 * 显示稀有度饼图和 6 星出货趋势图
 */
const ChartSection = ({ title, subtitle, color, data, isGlobal, tooltipStyle, isDark, mobile = false }) => {
  ...
  if (!data || data.total === 0) {
    return (
      <div className={`bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 min-w-0 ${mobile ? 'p-3' : 'p-6'}`}>
        <h3 className={`font-bold ${mobile ? 'text-xs' : 'text-lg'} ${color} mb-4`}>{title}</h3>
        <div className={`${mobile ? 'h-28 text-xs' : 'h-48'} flex items-center justify-center text-zinc-400`}>
  ...
  if (isGlobal && !hasDetailedData) {
    return (
      <div className={`bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 min-w-0 ${mobile ? 'p-3' : 'p-6'}`}>
        <div className={`flex items-center gap-2 mb-3 ${mobile ? 'flex-wrap' : ''}`}>
          <h3 className={`font-bold ${mobile ? 'text-xs' : 'text-lg'} ${color}`}>{title}</h3>
          {subtitle && <span className="text-[10px] text-zinc-500">({subtitle})</span>}
          <span className={`ml-auto font-mono ${mobile ? 'text-[10px]' : 'text-sm'} text-zinc-500`}>{formatNumber(data.total || 0)} {tt('summary.metric.pullsUnit', '抽')}</span>
        </div>
        <div className={`${mobile ? 'h-24' : 'h-32'} flex items-center justify-center text-zinc-500 bg-zinc-50 dark:bg-zinc-950 border border-dashed border-zinc-300 dark:border-zinc-700`}>
          <div className="text-center">
            <Cloud size={mobile ? 22 : 32} className="mx-auto mb-2 opacity-50" />
            <p className={mobile ? 'text-xs' : 'text-sm'}>{tt('summary.notice.globalOnly', '全服统计数据')}</p>
            <p className="text-[10px] text-zinc-400 mt-1">{tt('summary.notice.globalOnlyHint', '详细图表请切换到「我的数据」查看')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 relative group hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors min-w-0">
      ...
      <div className={`${mobile ? 'p-3' : 'p-5'} border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-white/5 flex items-center justify-between gap-2`}>
        <div className={`flex min-w-0 items-center ${mobile ? 'gap-2' : 'gap-3'}`}>
          <div className={`w-1 ${mobile ? 'h-3' : 'h-4'} ${color.replace('text-', 'bg-')}`}></div>
          <h3 className={`truncate font-bold ${mobile ? 'text-xs' : 'text-base'} ${color} uppercase tracking-wider`}>{title}</h3>
          {subtitle && <span className={`${mobile ? 'hidden' : 'text-xs'} text-zinc-400 dark:text-zinc-500 font-mono border-l border-zinc-200 dark:border-zinc-700 pl-3`}>{subtitle}</span>}
        </div>
        <div className={`${mobile ? 'text-[10px]' : 'text-xs'} flex shrink-0 items-center gap-1.5 font-mono text-zinc-500`}>
          <span>{tt('summary.metric.totalShort', '总计')}</span>
          <span className="bg-zinc-200 dark:bg-zinc-800 px-1.5 py-0.5 rounded-sm text-zinc-700 dark:text-zinc-300 font-bold min-w-[2.5rem] text-center">
            {formatNumber(data.total || 0)}
          </span>
        </div>
      </div>

      <div className={`${mobile ? 'p-3 gap-4' : 'p-6 gap-8'} grid grid-cols-1 md:grid-cols-2 min-w-0`}>
        {/* 饼图 */}
        <div className={`${mobile ? 'h-44' : 'h-52'} relative min-w-0`}>
          <p className="text-[10px] font-bold text-zinc-500 mb-2">{tt('summary.section.rarityDistribution', '稀有度分布')}</p>
          {hasChartData ? (
            <div className="flex h-full items-center justify-center overflow-hidden">
              <PieChart width={mobile ? 280 : 320} height={mobile ? 176 : 208}>
                <RainbowGradientDefs />
                <Pie
                  data={data.chartData}
                  cx="50%"
                  cy="45%"
                  innerRadius={mobile ? 26 : 32}
                  outerRadius={mobile ? 58 : 72}
                  paddingAngle={2}
