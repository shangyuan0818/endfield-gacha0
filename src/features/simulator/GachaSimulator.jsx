...
const GachaSimulator = () => {
  ...
  useEffect(() => {
    ...
    if (targetPool && targetPoolId) {
      ...
      const newSim = createSimulator(targetPool.type, getCustomRulesForPool(targetPool), upChar, poolCharactersList);
      if (savedState) {
  ...
  const switchPool = (poolId) => {
    ...
    const newSim = createSimulator(targetPool.type, getCustomRulesForPool(targetPool), upChar, null);
    if (savedState) {
      newSim.importState(savedState);
