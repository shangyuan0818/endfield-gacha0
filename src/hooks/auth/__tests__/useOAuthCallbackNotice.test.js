...
import {
  buildOAuthAccountCompletionNotice,
  buildOAuthNotice,
  useOAuthCallbackNotice,
} from '../useOAuthCallbackNotice.js';
...
vi.mock('../../../services/accountSecurityService.js', () => ({
  isOAuthAccountCompletionRequired: vi.fn(),
...
describe('useOAuthCallbackNotice', () => {
  beforeEach(() => {
    ...
    loadAccountSecurityState.mockResolvedValue({});
    isOAuthAccountCompletionRequired.mockReturnValue(false);
  });
  ...
  it('syncs the site session and emits a shared event after OAuth sign-in', async () => {
    ...
    const onSessionSynced = vi.fn().mockResolvedValue(true);
    ...
    await waitFor(() => {
      ...
      expect(loadAccountSecurityState).toHaveBeenCalled();
      expect(isOAuthAccountCompletionRequired).toHaveBeenCalledWith({});
    });

    window.removeEventListener(AUTH_SESSION_SYNC_EVENT, eventListener);
  });

  it('adds an account completion notice after OAuth sign-in when setup is still required', async () => {
    const navigate = vi.fn();
    const addDurableNotification = vi.fn();
    const securityState = {
    ...
    loadAccountSecurityState.mockResolvedValue(securityState);
    isOAuthAccountCompletionRequired.mockReturnValue(true);
    ...
    await waitFor(() => {
      expect(loadAccountSecurityState).toHaveBeenCalled();
      expect(isOAuthAccountCompletionRequired).toHaveBeenCalledWith(securityState);
      expect(addDurableNotification).toHaveBeenCalledWith(expect.objectContaining({
        type: 'warning',
        title: '请补全账号安全信息',
  ...
  it('does not treat the legacy verified callback status as a usable login session', async () => {
    ...
    expect(loadAccountSecurityState).not.toHaveBeenCalled();
  });
});

describe('buildOAuthAccountCompletionNotice', () => {
  it('points users to Settings in Chinese', () => {
    expect(buildOAuthAccountCompletionNotice(false)).toMatchObject({
      type: 'warning',
      category: 'account',
      title: '请补全账号安全信息',
      actions: [
...
describe('buildOAuthNotice', () => {
  ...
  it('explains unlinked provider sign-in failures as a rebind flow', () => {
    const notice = buildOAuthNotice({
      status: 'error',
      provider: 'github',
      code: 'oauth_identity_unlinked',
    }, false);

    expect(notice.title).toBe('GitHub 已被解绑');
    expect(notice.message).toContain('邮箱和密码登录');
    expect(notice.message).toContain('设置页重新绑定');
    expect(notice.actions).toEqual([
      { label: '打开设置', href: '/settings', variant: 'primary' },
    ]);
    expect(notice.diagnostic).toEqual({
