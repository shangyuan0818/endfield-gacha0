...
第三方一键登录分两层：GitHub 交给自建 Supabase Auth / GoTrue 生成真实登录态，浏览器端通过 `supabase.auth.signInWithOAuth({ provider: 'github' })` 跳转，并在 `/auth/callback` 调用 `exchangeCodeForSession()` 完成会话交换；Linux.do 使用 Supabase Custom OAuth2 provider，provider id 为 `custom:linuxdo`，换 token 和用户资料端点分别为 `https://connect.linux.do/oauth2/token`、`https://connect.linux.do/api/user`，scope 为 `read`。自建 Auth 当前会把 Custom Provider 的回调生成为 `https://db.15963574.xyz/callback`，所以自建 Supabase 网关必须保留 `/callback -> auth:9999/callback` 的公开路由，Linux.do 应用后台也应登记这个回调地址；授权端点通过本站 `/api/auth/oauth/linuxdo/supabase-authorize` 代理转发到 `https://connect.linux.do/oauth2/authorize`，只保留标准 OAuth2 参数，避免 Supabase 额外带出的 `redirect_to` 被 Linux.do 拒绝。自建 Auth 需要使用包含 `custom_oauth_providers` 迁移和 `/admin/custom-providers` 接口的版本；完成 provider 创建并确认 authorize 链路能跳转到 Linux.do 后，才能同时打开 `VITE_AUTH_OAUTH_LINUXDO_ENABLED=true` 和 `VITE_AUTH_OAUTH_LINUXDO_READY=true`。QQ 暂时保留本站桥接入口 `/api/auth/oauth/qq/start` 和 `/api/auth/oauth/qq/callback`，等 QQ 应用审核和真实返回格式确认后再开放。GitHub / Linux.do 的真实 secret 只写入 Supabase Auth 容器或 Auth 配置，不进入 Vercel 前端变量；手动绑定需要 Supabase Auth 开启 `GOTRUE_SECURITY_MANUAL_LINKING_ENABLED=true`。旧桥接仍可用于 QQ 和诊断兜底，服务端必须继续签名 `state`、限制回跳地址，并避免保存 raw access token / refresh token 或把 provider 原始资料写入公开输出。
